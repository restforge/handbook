...
- Kolom audit (`created_at`, `created_by`, `updated_at`, `updated_by`) tidak perlu didaftarkan di `fieldValidation` karena ditangani secara otomatis oleh runtime. Detail di [catalogs/rdf/audit-columns.md](../../catalogs/rdf/audit-columns.md).
...
2. Sesuaikan `tableName`, kolom di `fieldName`, dan aturan di `fieldValidation` agar cocok dengan tabel database
...
4. Generate endpoint dengan command:
   ...
   ```bash
   npx restforge endpoint create --project=<NAMA-PROJECT> --name=<NAMA-ENDPOINT> --payload=<nama-file-payload>
   ```

   Flag `--payload` menerima nama file payload, dengan atau tanpa `.json`.
