# Sistem Constraint (Constraint System)
...
Function `now()` di-translate oleh DDL generator ke native function per dialect:
...
Auto-create single column index pada field tersebut. Alternatif lebih singkat dari mendaftarkan field di array `indexes`.
...
> **STATUS: DEPRECATED.** Constraint ini tidak boleh digunakan di template baru. Engine masih menerima token ini untuk backward compatibility, namun token tersebut tidak memiliki efek fungsional apapun.
...
### Limitations Modifier `autoUpdate` (Mengapa Di-Deprecate)

Investigasi source code menunjukkan modifier ini secara fungsional tidak memberikan kontribusi apapun:
...
- **Tidak ter-recover** dari introspeksi database (roundtrip SDF → DB → SDF akan kehilangan marker).
- **Tidak ada consumer aktif** di codebase yang membaca flag `field.autoUpdate` dari IR untuk menghasilkan behavior berbeda. Runtime BaseModel meng-inject `updated_at = CURRENT_TIMESTAMP` berdasarkan nama kolom audit standar, bukan berdasarkan marker.
...
Tidak ada loss of functionality karena auto-update tetap bekerja di runtime tanpa marker tersebut.
...
- Hanya valid untuk type `timestamp` dan `date`. Type lain akan throw error.
- Tidak boleh dikombinasikan dengan `default:` shorthand (misal `'timestamp default:now() autoUpdate'` akan throw error).
...
PostgreSQL dan Oracle tidak otomatis menghasilkan index untuk foreign key column, sehingga eksplisitnya generator dibutuhkan. MySQL otomatis menghasilkan index untuk FK, namun generator tetap emit DDL eksplisit untuk konsistensi lintas dialect.

## Aturan Skip Emit untuk Index Redundan

Bila user mendeklarasikan field di `indexes` array tetapi kolom tersebut sudah punya implicit index dari sumber lain, generator akan skip emit `CREATE INDEX` untuk menghindari duplikasi. Sumber implicit index:
...
Penulisan `index` eksplisit di field shorthand atau di array `indexes` tetap diperbolehkan dan tidak akan menyebabkan error (parser tidak throw). Generator akan deduplicate berdasarkan kolom dan skip yang redundan.
