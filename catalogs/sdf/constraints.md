...
| MySQL | `CURRENT_TIMESTAMP(3)`, presisinya sama dengan kolom `DATETIME(3)` |
| Oracle | `SYSTIMESTAMP` |
...
fields: {
  ...
  clockin:        'timestamptz default:now()'
}
```
...
> **STATUS: DEPRECATED.** Constraint ini tidak boleh digunakan di template baru. RESTForge masih menerima token ini untuk backward compatibility, namun token tersebut tidak memiliki efek fungsional apa pun.
...
Auto-update untuk field `updated_at` ditangani sepenuhnya oleh runtime melalui konvensi `auditColumns` di **RDF** (Resource Definition File). Mekanisme ini bekerja berdasarkan **konvensi nama field**, bukan berdasarkan marker SDF. Detail penanganan dapat dibaca di [`rdf/audit-columns.md`](../rdf/audit-columns.md).

### Keterbatasan Modifier `autoUpdate`

Modifier ini tidak memberikan efek apa pun pada database maupun runtime:

- **Output DDL identik** dengan `'timestamp'` biasa. Tidak ada `ON UPDATE` clause, tidak ada trigger, tidak ada DEFAULT.
- **Tidak terdeteksi oleh `schema diff`** sehingga penambahan atau penghapusan marker tidak menghasilkan migration script.
- **Tidak dapat dipulihkan** dari introspeksi database (roundtrip SDF → DB → SDF akan kehilangan marker).
- **Tidak memengaruhi runtime.** Runtime mengisi `updated_at` dengan waktu saat ini menurut `TIMEZONE` berdasarkan nama kolom audit standar, bukan berdasarkan marker.
...
### Aturan Validasi yang Masih Berlaku

Untuk template lama yang masih menggunakan modifier ini, validator masih memberlakukan aturan berikut:

- Hanya valid untuk type `timestamp`, `timestamptz`, atau `date`. Type lain akan
  melempar error: `field '<field>' constraint 'autoUpdate' only valid for type
  'timestamp', 'timestamptz', or 'date', got '<type>'`.
