...
| `relations[*].onDelete` harus enum (`cascade`, `restrict`, `setNull`, `noAction`) | `Relation '<name>' has invalid onDelete '<value>'` |
...
| Constraint `autoUpdate` hanya valid untuk type `timestamp`, `timestamptz`, atau `date` (DEPRECATED rule, masih aktif untuk backward compatibility) | `field '<field>' constraint 'autoUpdate' only valid for type 'timestamp', 'timestamptz', or 'date', got '<type>'` |
...
> **Catatan**: Aturan validasi untuk constraint `autoUpdate` masih berlaku untuk backward compatibility. Constraint ini sudah DEPRECATED dan tidak boleh dipakai di template baru. Auto-update field `updated_at` ditangani runtime melalui konvensi `auditColumns` di RDF, bukan di SDF. Detail lengkap dibahas di [`constraints.md`](./constraints.md#constraint-autoupdate-deprecated).
...
Verb `schema validate` hanya menerima flag `--schema-path`, dan satu-satunya bentuk output yang tersedia adalah laporan teks untuk dibaca manusia. Output terstruktur untuk konsumsi MCP, CI, atau LSP masih berupa rencana: kontrak datanya sudah disiapkan di layer library dengan bentuk `{ schemaVersion: '1.0', summary: { modelsLoaded, errorCount, warningCount }, issues: [...] }`, tetapi belum ada flag CLI yang mengeluarkannya. Verb serumpun `schema diff` sudah punya jalur JSON lewat flag boolean `--json`.
