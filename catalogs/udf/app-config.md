...
Nilai `port` hanya mempengaruhi HTTP server statis yang dijalankan oleh `app-start.bat` (memakai `npx serve`). Properti ini **tidak** mempengaruhi `apiBaseUrl` atau koneksi ke backend API.
