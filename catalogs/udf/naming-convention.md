...
Pattern ini memblokir path traversal karena `pageId` dipakai sebagai filename HTML output. Tanpa validasi pattern, user yang malicious bisa menulis `pageId` yang menghasilkan file di lokasi tidak diinginkan.
...
Generator mengkonversi `name` field secara otomatis ke format lain untuk berbagai kebutuhan:
...
- Mudah dikonversi otomatis ke kebab-case dan camelCase
