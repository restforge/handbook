...
Live Sync menghubungkan aplikasi frontend ke server WebSocket. Ketika ada event perubahan data dari client lain (mis. user lain menyimpan record), tabel data secara otomatis di-reload tanpa user perlu klik refresh manual.
...
Halaman `stock-balance` di atas mendapat tombol Refresh di toolbar beserta penanda waktu `Updated HH:MM · live`. Halaman `master-product` tanpa Live Sync tidak punya tombol Refresh.
...
3. JavaScript menerima event, memvalidasi `pageId`, dan memicu reload DataTable
...
| Dashboard real-time | Gunakan `refreshInterval` (lihat [`dashboard-page.md`](./dashboard-page.md#refreshinterval)) |
| Data berubah oleh pihak lain tanpa server WebSocket | Gunakan `features.autoRefresh` (lihat [`features.md`](./features.md#autorefresh)) |
