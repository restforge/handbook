...
| Status = Draft | Field `approval_date` disembunyikan |
| Tipe transaksi = Pembelian Kredit | Field `due_date` tampil; tipe Tunai → disembunyikan |
...
Berlaku: hanya `due_date` dan `installment_count` yang disembunyikan ketika `payment_type == 'cash'`.
...
| `order_status` | Hasil |
|----------------|-------|
| `'draft'` | `approval_date` dan `approved_by` disembunyikan. Field lain editable |
...
`fieldStates` sangat sering dipakai bersama `workflow` + `workflowActions[]` untuk mengunci field setelah transisi status:
...
Behavior: setelah aksi Approve berhasil, semua field jadi readonly secara otomatis.
