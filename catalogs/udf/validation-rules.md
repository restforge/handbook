...
Validator UDF dijalankan oleh tiga command berikut:
...
Generator mendukung dua scope yang memengaruhi behavior validasi:
...
| `appConfig.numberFormat.decimalPlaces` sudah dihapus | Error | `appConfig.numberFormat.decimalPlaces has been removed; the number of decimal places is now a field attribute. Set 'decimalPlaces' on the field instead (pages[].fields[].decimalPlaces, derived by \`payload migrate\` from the column scale)` |
| `appConfig.numberFormat.locale` salah satu locale yang didukung | Error | `appConfig.numberFormat.locale must be one of: en-US, id-ID, got '<value>'` |
| `appConfig.numberFormat.currencyPrefix` harus string jika ada | Error | `appConfig.numberFormat.currencyPrefix must be a string when provided, got <type>` |
| `appConfig.dateFormat` pola tanggal yang didukung | Error | `appConfig.dateFormat must be one of: yyyy-MM-dd, dd/MM/yyyy, dd-MM-yyyy, MM/dd/yyyy, yyyy/MM/dd, got '<value>'. It must match the backend DATEFORMAT in config/db-connection.env` |
| `appConfig.dateTimeFormat` berbentuk `<pola tanggal> <pola waktu>` yang didukung | Error | `appConfig.dateTimeFormat must be '<date pattern> <time pattern>' with a date pattern from ... and a time pattern from ..., got '<value>'. It must match the backend DATETIMEFORMAT in config/db-connection.env` |
...
| `temporalType` valid (`timestamp`/`timestamptz`) | Error | `Field '<name>' has an invalid temporalType: '<value>'. Valid values are: timestamp, timestamptz` |
| `temporalType` hanya pada field `timestamp` | Error | `Field '<name>': temporalType applies only to fields of type 'timestamp'` |
| `valueFormat` tidak lagi didukung | Error | `Field '<name>': valueFormat is no longer supported. Record values always follow appConfig.dateFormat and appConfig.dateTimeFormat (the backend DATEFORMAT and DATETIMEFORMAT); remove valueFormat and run payload migrate again` |
| `dateFormat` field pola tanggal yang didukung | Error | `Field '<name>' has an invalid dateFormat "<value>": expected a date pattern, one of: yyyy-MM-dd, dd/MM/yyyy, dd-MM-yyyy, MM/dd/yyyy, yyyy/MM/dd` |
...
| `decimalPlaces` integer 0–20 (bila ada) | Error | `Field '<name>' decimalPlaces must be an integer between 0 and 20, got <value>` |
...
1. **Pakai `rfd validate` standalone** sebelum `generate` untuk mendapatkan output validation yang lebih jelas
...
4. **Error muncul di urutan struktural**, bukan severity. Field error mungkin muncul setelah error lain di bagian yang sama
5. **Tidak ada validasi sintaks ekspresi `when`** di `fieldStates[]`. Pastikan sintaks JavaScript valid secara manual
