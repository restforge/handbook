...
Field selain di atas di-passthrough sebagai metadata tambahan dan dapat dibaca oleh plugin tertentu (mis. plugin `vanilla-js-auth` membaca field `auth` di level root).
...
**File yang di-include wajib berupa fragment UDF yang memuat array `pages`**, bukan object page polos. Saat load, loader membaca file yang ditunjuk, **mengekstrak array `pages` di dalamnya**, lalu menyisipkan (append) page-page tersebut menggantikan entry `include`. Path bersifat relatif terhadap file UDF utama.
...
|----------|--------------|--------|
| `appConfig` | File base | Diambil seluruhnya dari file yang di-extends |
| `pages` | File payload | Diambil dari file payload — tidak di-merge dengan base |
