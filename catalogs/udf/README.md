...
| Single source of truth per aplikasi | Satu file UDF (atau satu set file dengan `extends`) menjadi referensi untuk seluruh aplikasi frontend |
...
| Modular page | Halaman bisa di-include via `{"include": "path"}` untuk pemecahan file per modul |
...
- Definisi endpoint REST API backend (di-handle oleh RDF di [`../rdf/`](../rdf/))
- Custom business logic non-CRUD di backend (di-handle oleh processor RDF, lihat [`../rdf/processor.md`](../rdf/processor.md))
- Logic JavaScript custom selain yang di-generate oleh plugin

## Struktur File UDF (UDF File Structure)
...
## Anatomi UDF (UDF Anatomy)

Daftar field top-level yang dikenali oleh generator dan validator, di-grup berdasarkan tingkat kepentingan.
...
> **Catatan:** Konversi RDF → UDF dilakukan oleh binary backend (`restforge`), bukan oleh binary frontend (`restforge-designer`). Alasan: konversi memerlukan akses ke konfigurasi database backend (mis. `SERVER_PORT`, `SERVER_ADDRESS`) yang lebih natural di-handle di sisi backend.
