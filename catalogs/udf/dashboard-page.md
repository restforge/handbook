...
pageSubtitle, pageTitle, pageType, refreshInterval, rows
```
...
Interval pemuatan ulang otomatis untuk seluruh widget dashboard, dalam detik.
...
|----------|------|---------|:-----:|-----------|
| `refreshInterval` | integer | `300` | ✗ | Jeda reload otomatis dalam detik, rentang 15 sampai 3600 |

Behavior:

- Header dashboard memuat satu tombol Refresh dan satu penanda waktu
- Seluruh widget dimuat ulang otomatis sesuai interval, tanpa mengosongkan widget saat reload
- Pemuatan otomatis ditunda saat tab browser tersembunyi atau saat kursor berada di atas widget
- Widget yang gagal dimuat mempertahankan data lama, ditandai peringatan kecil pada kartu widget tersebut
- Penanda waktu hanya diperbarui bila seluruh widget berhasil dimuat

Nilai di luar rentang menghasilkan error:

```
[<pageId>] refreshInterval must be an integer between 15 and 3600 (seconds)
```
...
#### `chart.format` (Format Angka per Lokasi)

Format angka untuk 4 lokasi yang berbeda di chart:
...
Field `chart` yang sudah dihapus di v5.x (akan menghasilkan error wajib migrasi):
