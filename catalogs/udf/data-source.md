...
"optionColumns": [
    { "column": "product_name" },
    { "column": "price", "format": "number", "decimalPlaces": 2, "align": "right" }
]
```
...
| `decimalPlaces` | integer | ✗ | Jumlah digit desimal, khusus entri ber-`format: "number"`. Integer 0–20, default `0`. Berlaku di dropdown maupun tabel modal. Tidak diisi `payload migrate` karena kolom lookup adalah keputusan tampilan manual |
...
| `dataSource.optionColumns[].decimalPlaces` integer 0–20 (bila ada) | Error: `"Field '<name>' dataSource.optionColumns column '<column>' decimalPlaces must be an integer between 0 and 20, got <value>"` |
