...
|----------|------|-------|-----------|
| `id` | string | Ya (jika `fieldNameLookup` di-set) | Kolom yang dipakai sebagai `id` di response |
| `text` | string | Ya (jika `fieldNameLookup` di-set) | Kolom atau SQL expression untuk `text` di response. Mendukung concatenation native dialect (PostgreSQL pakai `||`, MySQL pakai `CONCAT()`) |
