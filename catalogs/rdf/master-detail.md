...
Konfigurasi untuk operasi composite (header + detail dalam satu transaksi). Wajib di-set jika action `createComposite`, `updateComposite`, atau `readComposite` aktif.
...
| `headerCalculations` | object | Field header yang dihitung otomatis dari detail items |
...
| Property | Tipe | Keterangan |
|----------|------|-----------|
| `type` | enum | `"calculated"` (dihitung app code, masuk SQL) atau `"generated"` (database GENERATED COLUMN, di-exclude dari SQL) |
...
Field header yang di-hitung **oleh application code** dari detail items saat create/update composite.
