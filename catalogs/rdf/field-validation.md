...
| `timestamp` | TIMESTAMP (PostgreSQL/Oracle/SQLite), DATETIME (MySQL) | `"2026-01-16 08:30:45"` |
| `datetime` | Alias `timestamp` — diterima sebagai masukan; generator meng-emit `timestamp` | `"2026-01-16 08:30:45"` |
| `time` | TIME | `"14:30:00"` |
| `uuid` | UUID | `"018f1a3b-7c9d-7abc-9def-0123456789ab"` (UUID v7, RFC 9562) |
| `json` | JSON, JSONB | `{"key": "value"}` |
| `array` | ARRAY | `[1, 2, 3]` |

### Catatan Tipe `timestamp` dan `datetime`

`timestamp` adalah tipe kanonik untuk kolom tanggal-waktu, selaras dengan sistem tipe SDF (SDF
`timestamp` menghasilkan DDL `TIMESTAMP`). `datetime` diterima sebagai alias masukan yang identik:
runtime memperlakukannya sama persis, dan kolom `DATETIME` MySQL dinormalisasi ke `timestamp` saat
introspeksi. Generator (`payload generate`) selalu meng-emit `timestamp`, baik di `fieldValidation`
maupun di [`dateTimeFields`](./datetime-fields.md). Untuk pemrosesan nilai waktu (normalisasi input,
pemformatan output, mode lenient/strict), lihat [`datetime-fields.md`](./datetime-fields.md).
