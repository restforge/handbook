...
| `dateTimeFields` | object | Konfigurasi field bertipe waktu (date/timestamp/time): normalisasi input, pemformatan output, mode lenient/strict via `format`. Lihat [`datetime-fields.md`](./datetime-fields.md) |
...
| `uploadConfig` | object | Konfigurasi field penerima file upload beserta batasan tipe, ukuran, dan jumlah file. Lihat [`upload-config.md`](./upload-config.md) |
...
| `authGuard` | object | Proteksi autentikasi + otorisasi endpoint (verifikasi JWT, permission per endpoint, path publik). Lihat [`auth-guard.md`](./auth-guard.md) |
...
| `concurrency` | object | Optimistic concurrency control berbasis versi baris untuk `update`/`update-composite` |
| `fieldPolicy` | object | Strategi per-kolom untuk row locking (`lock`) dan audit trail (`audit`) pada operasi tulis |
...
| Field DateTime (`dateTimeFields`) | [`datetime-fields.md`](./datetime-fields.md) |
...
| Field Auth Guard (`authGuard`) | [`auth-guard.md`](./auth-guard.md) |
| Field Concurrency (`concurrency`) | [`concurrency.md`](./concurrency.md) |
| Field Policy (`fieldPolicy`) | [`field-policy.md`](./field-policy.md) |
...
| Field Upload Config (`uploadConfig`) | [`upload-config.md`](./upload-config.md) |
