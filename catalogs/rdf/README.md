...
1. Mendeklarasikan struktur endpoint REST API (tabel target, kolom yang di-expose, action yang aktif)
...
Satu file RDF di-map ke satu endpoint resource di runtime. Path fisik konvensi: `payload/<resource>.json` di root project.
...
RDF bekerja sebagai input untuk generator. Hasil generation adalah module JavaScript yang langsung di-mount oleh runtime server tanpa kompilasi ulang.
...
| Single source of truth per resource | Satu file RDF menjadi referensi untuk seluruh endpoint resource (datatables, create, update, delete, dst) |
...
### Lingkup yang TIDAK Dicover

- Definisi struktur tabel database (di-handle oleh SDF di [`../sdf/`](../sdf/))
- Definisi halaman UI frontend (di-handle oleh UDF di [`../udf/`](../udf/))
- Custom business logic non-CRUD (di-handle oleh processor terpisah, lihat [`processor.md`](./processor.md))

## Struktur File RDF (RDF File Structure)
...
## Anatomi RDF (RDF Anatomy)

Berikut daftar lengkap field top-level yang dikenali oleh generator. Field di-grup berdasarkan tingkat kepentingan.
...
| `fieldName` | array of string | Daftar kolom yang di-expose sebagai field API. Urutan menentukan prioritas tampilan |
| `action` | object | Konfigurasi endpoint yang akan di-generate (setiap key = flag boolean per action) |

### Field Opsional Umum (Common Optional Fields)
...
| `exportQuery` | string | SQL SELECT khusus untuk endpoint `/export`. Default: fallback ke `datatablesQuery` |
...
### Field Konfigurasi Fitur (Feature Configuration Fields)
