...
Mengontrol behavior auto-injection kolom audit (`created_at`, `created_by`, `updated_at`, `updated_by`) pada operasi `create` dan `update`. Secara default RESTForge mengasumsikan tabel memiliki empat kolom audit standar tersebut dan secara otomatis menyisipkan value-nya ke INSERT/UPDATE statement. Field `auditColumns` digunakan untuk override behavior tersebut.

## Tiga Mode Konfigurasi

| Nilai | Perilaku Generator | Use Case |
|-------|-------------------|----------|
| Tidak ditetapkan (default) | Generator pakai default BaseModel: inject `created_at`, `created_by`, `updated_at`, `updated_by` saat INSERT/UPDATE | Tabel mengikuti konvensi audit standar RESTForge |
...
Digunakan saat tabel target **tidak** memiliki kolom audit standar. Tanpa flag ini, runtime akan mencoba menyisipkan kolom audit ke INSERT/UPDATE dan menyebabkan error 500 karena kolom tidak ada.
...
Generator akan menyisipkan kolom audit ke INSERT/UPDATE memakai nama kolom override (`tanggal_buat`, `dibuat_oleh`, dst), bukan default RESTForge.
...
Saat perintah `restforge payload generate` membaca skema database, generator secara otomatis mendeteksi keberadaan kolom audit standar:
...
| Tidak boleh array | (Sama dengan di atas, array diperlakukan sebagai value invalid) |
...
Field audit tidak perlu didaftarkan di [field-validation.md](./field-validation.md) karena value-nya disisipkan oleh runtime, bukan dari request body. Bila ingin mengekspos kolom audit di response (misal di `/datatables` atau `/first`), tetap daftarkan nama kolom di `fieldName`.
