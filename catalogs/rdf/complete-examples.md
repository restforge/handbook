...
Kolom `category_name` pada contoh ini berasal dari JOIN di `viewQuery` dan `datatablesQuery`, bukan kolom fisik tabel `item_product`. Kolom seperti itu sengaja didaftarkan di `fieldName` agar ikut muncul di response dan dapat dipakai sebagai target pencarian lewat `datatablesWhere`. Peran ganda itu berhenti di jalur baca: kolom hasil JOIN tidak pernah ditulis oleh `/create` maupun `/update`, dan nilainya diabaikan diam-diam bila ikut terkirim di request body. Karena itu pula `category_name` tidak muncul di `fieldValidation` dan tidak dapat dipakai di `adjustConfig.fields`.
