...
|----------|------|---------|-----------|
| `statusField` | string | `"status"` | Nama kolom database yang menyimpan status. Wajib ada di `fieldName` |
...
Object dengan key = status saat ini, value = array status tujuan yang diizinkan. Status tujuan yang tidak terdaftar ditolak dengan HTTP 422 `Status transition not allowed`.
