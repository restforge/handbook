...
| `codegen_validate_payload` | `payload validate --config=<config>` | `config` (default `db-connection.env`), `table` | Validasi RDF terhadap catalog dan struktur database. Tanpa `table`, seluruh payload di folder diperiksa. Validasi struktur RDF berjalan lebih dulu daripada perbandingan ke database, termasuk pengecekan entri `datatablesWhere`; file yang gagal di tahap itu memperoleh verdict `[ERROR]` dan perbandingan schema-nya dilewati. |
