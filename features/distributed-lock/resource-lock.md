...
| `LOCK_DISTRIBUTED_ENABLED=true` | Wajib ditetapkan agar endpoint resource lock terdaftar |
...
Per-record lock otomatis **tidak melindungi** `createComposite`, karena header record belum ada saat INSERT sehingga tidak ada PK yang dapat di-lock. Untuk skenario `createComposite` yang melibatkan shared resource (misalnya validasi stok), gunakan Resource Lock API dari sisi client:
