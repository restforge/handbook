...
| INSERT | Hanya kolom terdaftar yang sekaligus kolom fisik tabel yang masuk ke statement INSERT |
| UPDATE | Hanya kolom terdaftar yang sekaligus kolom fisik tabel yang masuk ke statement UPDATE |
| WHERE | Hanya kolom terdaftar yang juga di-output sumber baca endpoint bersangkutan yang boleh dipakai sebagai filter |
| ORDER BY | Hanya kolom terdaftar yang boleh dipakai sebagai pengurutan |

Pada operasi write, daftar `fieldName` disaring sekali lagi terhadap kolom fisik tabel, sehingga kolom hasil JOIN tidak pernah ikut ditulis. Pada operasi filter, daftar kolom yang boleh dipakai berbeda per endpoint: `/read` dan `/first` memakai kolom hasil sumber baca resolusi, sedangkan `/datatables` memakai kolom hasil `datatablesQuery`. Rinciannya ada di [`api-spec/endpoint-read.md`](../../api-spec/endpoint-read.md) dan [`api-spec/endpoint-datatables.md`](../../api-spec/endpoint-datatables.md).
...
| Kolom JOIN didaftarkan di `fieldName` lalu ikut dikirim di body `/create` atau `/update` | Nilainya diabaikan diam-diam. Kolom JOIN bersifat baca dan cari saja, tidak pernah ditulis ke tabel |
...
Sanitasi: trim, uppercase, default
        ↓
Hanya field fieldName yang juga kolom fisik tabel yang masuk ke SQL
```
...
|----------|:----------------:|:---------------------:|:------------------:|
| `fieldName` | Tidak | Ya, filter kolom output | Ya, filter kolom INSERT/UPDATE bersama kolom fisik tabel |
