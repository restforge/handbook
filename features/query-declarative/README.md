...
| Resolusi `/read-composite` | Header sama dengan `/read`; detail memakai `detailQuery` lalu fallback berkolom pengurut hasil resolusi |
...
/read-composite  →  Header: viewName  →  viewQuery  →  tableName
                    Detail: detailQuery  →  fallback: SELECT * FROM {detailTable}
                                           WHERE {foreignKey} = ?
                                           ORDER BY {kolom pengurut hasil resolusi}
```

Kolom pengurut fallback detail diresolusi saat generate: `line_number` bila kolom itu terdaftar di `masterDetail.detailConfig.fieldName`, selain itu primary key tabel detail. Lihat [Master-Detail Composite](master-detail.md#memahami-resolusi-composite-read-composite-resolution).
