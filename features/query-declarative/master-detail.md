...
| Detail | `masterDetail.detailConfig.detailQuery` | `SELECT * FROM {detailTable} WHERE {foreignKey} = ? ORDER BY {kolom pengurut hasil resolusi}` |

Kolom pengurut pada fallback detail diresolusi saat generate menurut aturan berikut: bila `detailConfig.fieldName` mencantumkan kolom bernama `line_number`, kolom itu yang dipakai, selain itu pengurutan memakai `detailConfig.primaryKey`. Kontrak lengkapnya ada di [`catalogs/rdf/master-detail.md`](../../catalogs/rdf/master-detail.md#kontrak-kolom-tabel-detail).
...
> **Catatan:** Apabila detail cukup dibaca apa adanya dari tabel detail berdasarkan foreign key header, `detailQuery` tidak perlu didefinisikan. Fallback otomatis sudah memadai, dan pengurutannya memakai `line_number` bila kolom itu terdaftar di `detailConfig.fieldName`, selain itu primary key tabel detail. Tabel detail tanpa kolom `line_number` karena itu tetap terbaca normal.
...
1. Prefix `file:` di-expand saat `endpoint create` dijalankan, sehingga konten SQL dibaca dari file pada tahap generation.
...
| Memerlukan urutan khusus selain kolom pengurut default | Ya |
