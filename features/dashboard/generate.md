...
| `--force=<bool>` | tidak | `false` | `true` untuk overwrite file existing (file lama diarsipkan ke `.restforge/archive/`) |
...
| `true` | File lama diarsipkan ke `.restforge/archive/<YYYYMMDD-HHMMSS>/src/modules/<project>/<name>.js`, lalu file baru ditulis |
...
| Row-level security native | Dashboard tidak menerapkan filter data per user secara otomatis; filter berbasis user identity dilakukan di middleware atau via SQL parameter eksplisit |
...
3. Bila file rusak atau diedit manual, generate ulang dengan `npx restforge dashboard create ... --force=true`, lalu restart server.
