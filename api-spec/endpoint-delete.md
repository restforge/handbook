...
Terjadi ketika record yang akan dihapus masih direferensikan oleh data di tabel lain (PostgreSQL error code `23503`). Pada endpoint delete, FK violation dipetakan ke 409 (bukan 400 seperti di create/update) karena ini merupakan konflik state, yaitu record tidak bisa dihapus selama masih digunakan.
...
## Perilaku pada Tabel Soft-Delete

Untuk resource dengan blok [`softDelete`](../catalogs/rdf/soft-delete.md) ber-`enabled: true` di RDF, endpoint `/delete` di-generate sebagai operasi soft-delete. Kontrak request (parameter `where`, body options) dan bentuk response sukses (`deleted_count`, `deleted_items`) tetap sama; yang berubah adalah operasi database dan semantik FK.

### Operasi yang Dijalankan

Alih-alih DELETE fisik, record di-UPDATE dalam satu transaction:

```sql
is_deleted = TRUE, deleted_at = NOW(), deleted_by = <user>
```

Nilai `deleted_by` di-resolve berurutan dari body eksplisit, lalu user context request, dengan fallback `'system'`. Kolom yang dideklarasikan `reusable` dimutasi menjadi `{nilai}##{uuidv7}` agar nilai aslinya dapat dipakai ulang oleh record baru tanpa melanggar UNIQUE.

### Semantik FK (FK-Aware)

| Situasi | Hasil |
|---------|-------|
| Record masih direferensikan anak `onDelete: restrict` | `409` dengan pesan `"Cannot delete: record is still referenced by '{childTable}' (onDelete: restrict)"` |
| Anak `onDelete: cascade` yang juga soft-delete | Ikut di-soft-delete secara rekursif (seluruh keturunan cascade) dalam transaction yang sama, all-or-nothing |

Untuk anak restrict yang juga soft-delete, hanya row anak yang masih **aktif** yang memblokir; row anak yang sudah terhapus tidak menghalangi. Anak restrict non-soft-delete memblokir dengan row apa pun.

### Pemulihan

Record yang ter-soft-delete tidak muncul lagi di endpoint read (sesuai `visibility`, default `active_only`) dan dapat dikembalikan melalui endpoint [`/restore`](endpoint-restore.md).
...
| Soft-Delete | [`catalogs/rdf/soft-delete.md`](../catalogs/rdf/soft-delete.md) | Override `/delete` menjadi penandaan flag + endpoint `/restore` |
