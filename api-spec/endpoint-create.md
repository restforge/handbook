...
Endpoint `/create` digunakan untuk menyisipkan satu record baru ke dalam tabel database. Request body berisi pasangan field-value yang akan di-insert. Primary key akan di-generate otomatis sebagai UUIDv7 jika tidak disertakan, bernilai kosong, atau diisi `"-"`.

Endpoint ini mendukung validasi deklaratif melalui konfigurasi `fieldValidation` di payload JSON, sehingga setiap field dapat divalidasi tanpa menulis kode tambahan. Setelah operasi berhasil, seluruh cache terkait endpoint ini akan di-invalidasi secara otomatis.
...
- String kosong `""` otomatis dikonversi ke `null` pada field yang namanya mengandung `date`, `time`, `timestamp`, `tags`, atau `json`
- Field datetime di-parse sesuai konfigurasi `dateTimeFields` di payload
...
Terjadi ketika ada error database atau error sistem yang tidak ter-handle.
...
Primary key dan audit columns diisi secara otomatis oleh sistem. Audit columns bersifat auto-managed (di-skip dari `fieldName` payload) dengan nama default `created_at`, `created_by`, `updated_at`, `updated_by`:

| Field | Nilai Otomatis | Kondisi |
|-------|---------------|---------|
| Primary key (e.g. `supplier_id`) | UUIDv7 | Jika tidak disertakan, kosong, atau bernilai `"-"` |
| `created_at` | SQL `CURRENT_TIMESTAMP` (waktu server) | Selalu di-set saat create |
| `created_by` | user_id hasil resolusi berurutan: nilai di request body → header `x-user-id` → fallback `"system"` | Selalu di-set saat create |
| `updated_at` | SQL `CURRENT_TIMESTAMP` (waktu server) | Ikut di-set saat create |
| `updated_by` | Sama dengan resolusi `created_by` | Ikut di-set saat create |
