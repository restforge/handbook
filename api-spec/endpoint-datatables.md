...
Endpoint `/datatables` dirancang untuk mengambil data dengan paginasi, pencarian, pengurutan, dan filtering. Response menggunakan format yang kompatibel dengan library jQuery DataTables maupun custom frontend.
...
| `searchBy` | string | Tidak | `"all"` | Harus ada di `datatablesWhere` | Kolom target pencarian. `"all"` = semua kolom |
| `sort_columns` | array | Tidak | Primary key ASC | Lihat [Format Sort Columns](README.md#format-sort-columns) | Pengurutan data |
| `where` | array / object | Tidak | `null` | Lihat [Format WHERE](README.md#format-where-clause) | Kondisi filter kompleks |
...
| `recordsTotal` | number | Jumlah total **seluruh** data tanpa filter apapun |
...
## Perbedaan dengan Endpoint /read (Comparison with /read)
