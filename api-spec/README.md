...
| 6 | [restore](endpoint-restore.md) | POST | `/api/{project}/{endpoint}/restore` | Memulihkan data soft-delete (membalik `/delete`) |
| 7 | [adjust](endpoint-adjust.md) | POST | `/api/{project}/{endpoint}/adjust` | Penyesuaian atomik field numerik |

### Workflow dan Status

| # | Endpoint | Metode | URL | Deskripsi |
|---|----------|--------|-----|-----------|
| 8 | [change-status](endpoint-change-status.md) | POST | `/api/{project}/{endpoint}/change-status` | Mengubah status record dengan validasi transisi + hook API |

### Query dan Retrieval

| # | Endpoint | Metode | URL | Deskripsi |
|---|----------|--------|-----|-----------|
| 9 | [datatables](endpoint-datatables.md) | POST | `/api/{project}/{endpoint}/datatables` | Server-side processing DataTables |
| 10 | [lookup](endpoint-lookup.md) | GET / POST | `/api/{project}/{endpoint}/lookup` | Data untuk dropdown/autocomplete |
| 11 | [aggregate](endpoint-aggregate.md) | POST | `/api/{project}/{endpoint}/aggregate` | Agregasi data (COUNT, SUM, AVG, MIN, MAX) |

### CRUD Composite — Master-Detail

| # | Endpoint | Metode | URL | Deskripsi |
|---|----------|--------|-----|-----------|
| 12 | [create-composite](endpoint-create-composite.md) | POST | `/api/{project}/{endpoint}/create-composite` | Membuat data header + detail items |
| 13 | [update-composite](endpoint-update-composite.md) | POST | `/api/{project}/{endpoint}/update-composite` | Memperbarui header + insert/update/delete detail |
| 14 | [read-composite](endpoint-read-composite.md) | POST | `/api/{project}/{endpoint}/read-composite` | Mengambil data header + detail items |

### Export dan Import

| # | Endpoint | Metode | URL | Deskripsi |
|---|----------|--------|-----|-----------|
| 15 | [export](endpoint-export.md) | POST + GET | `/api/{project}/{endpoint}/export` | Ekspor data ke Excel (.xlsx) |
| 16 | [import](endpoint-import.md) | POST + GET | `/api/{project}/{endpoint}/import-*` | Impor data dari Excel (.xlsx) |

### Upload File

| # | Endpoint | Metode | URL | Deskripsi |
|---|----------|--------|-----|-----------|
| 17 | [upload](endpoint-upload.md) | POST + GET | `/api/{project}/{endpoint}/upload-*` | Upload file binary ke object storage (AWS S3, S3-compatible, atau local) |
...
| `{action}` | Aksi yang dilakukan | `create`, `read`, `first`, `update`, `delete`, `restore`, `adjust`, `change-status`, `datatables`, `lookup`, `aggregate`, `create-composite`, `export`, dll. |
...
### Header Pola Tanggal-Waktu

Setiap response membawa tiga header yang menyebut pola tanggal-waktu yang sedang berlaku di
backend, sehingga klien dapat membaca dan mengirim nilai tanggal tanpa konfigurasi manual:

| Header | Isi | Contoh |
|--------|-----|--------|
| `X-Timezone` | Zona aplikasi (`TIMEZONE`) | `Asia/Jakarta` |
| `X-Date-Format` | Pola field `date` (`DATEFORMAT`) | `dd/MM/yyyy` |
| `X-DateTime-Format` | Pola field `timestamp` (`DATETIMEFORMAT`) | `dd/MM/yyyy HH:mm:ss.SSS` |

Field `timestamptz` selalu dikirim sebagai ISO UTC dan tidak mengikuti `X-DateTime-Format`.
Kontrak lengkapnya ada di [`catalogs/rdf/datetime-fields.md`](../catalogs/rdf/datetime-fields.md).
...
### Field yang Dikirim di Response

Record di response hanya memuat field yang terdaftar di `fieldName` payload. Aturan ini sama di PostgreSQL, MySQL, SQLite, dan Oracle, dan berlaku untuk `/read`, `/datatables`, `/first`, `/create`, `/update`, `/change-status`, serta header ketiga endpoint composite. Kolom tabel yang tidak terdaftar tidak pernah terkirim, termasuk kolom yang ditambahkan lewat migrasi database tanpa membuat ulang payload.

Kolom audit (`created_at`, `created_by`, `updated_at`, `updated_by`) tetap diisi runtime di database, tetapi `payload generate` tidak memasukkannya ke `fieldName`. Karena itu kolom audit baru muncul di response bila namanya didaftarkan di `fieldName`, lihat [`catalogs/rdf/audit-columns.md`](../catalogs/rdf/audit-columns.md#hubungan-dengan-fieldvalidation).

Item detail pada endpoint composite mengikuti `detailConfig.fieldName` ditambah primary key detail, foreign key, dan kolom `autoCalculateFields`. Pengecualiannya ada pada `/read-composite` dengan `detailQuery`, yaitu item detail berisi kolom yang dipilih query tersebut. Rinciannya ada di [endpoint-read-composite.md](endpoint-read-composite.md#konfigurasi-detail-query).

Beberapa key bukan kolom tabel dan tetap dikirim, yaitu `rownumerator` pada `/datatables` dan `_operations` pada `/update-composite`. Response `/delete` punya kontrak sendiri, lihat [endpoint-delete.md](endpoint-delete.md#format-response).
...
| `details` | string \| object | Dua bentuk: (1) **string** detail error teknis pada **500**, hanya muncul di development mode (`NODE_ENV=development`); (2) **object** map `{ "<kolom>": ["<pesan>"] }` pada **409 Duplicate entry**, selalu muncul bila kolom pelanggar dapat diekstrak (lihat bagian Duplicate entry) |
| `timestamp` | string | Waktu eksekusi dalam format ISO 8601 |
...
| **Grup WHERE bersarang tidak valid** | `Bad Request` | `Invalid where conditions: Invalid where format: nested group must have logic AND/OR and a non-empty conditions array` |
| **Grup WHERE bersarang terlalu dalam** | `Bad Request` | `Invalid where conditions: Invalid where format: nested group depth exceeds maximum of 10` |
...
| **`searchBy` tidak dikenal** | `Bad Request` | `Invalid searchBy: kategori. Valid searchBy values: all, product_code` (endpoint `/datatables`) |
| **Validasi field gagal** | `Validation failed` | `Invalid data` + object `errors` |
| **Nilai tanggal-waktu tidak valid** | `Invalid datetime` (disertai `code: "INVALID_DATETIME"`) | `Invalid datetime value '18/09/2026': type 'timestamp' requires a time component; a date-only value is not accepted` |
...
| **Penolakan hook component (change-status)** | `Workflow hook rejected` | `Stok tidak mencukupi: Baju 29 (tersedia 0, diminta 5)` (pesan asli handler `onBeforeWorkflow`/`onAfterWorkflow`, tanpa prefiks nama fungsi dan path file) |
...
- **Create/Update** → FK `23503` dipetakan ke **400** (data referensi tidak ditemukan = client error)
- **Delete** → FK `23503` dipetakan ke **409** (record tidak bisa dihapus karena masih digunakan = conflict)

**Catatan lock failure:** Pada `/update`, `/adjust`, `/delete`, kegagalan memperoleh lock dipetakan ke **500** (catch-all). Pada `/change-status`, kegagalan lock dipetakan khusus ke **409** `Resource busy`.

**Field `details` pada Duplicate entry:** response **409 Duplicate entry** menyertakan map `details` berbentuk `{ "<kolom>": ["<pesan>"] }` yang menunjuk kolom yang melanggar unique constraint, berguna untuk memetakan error ke field form. Untuk unique constraint multi-kolom, setiap kolom dipetakan ke pesan `"A record with this combination of values already exists"`. Berbeda dengan `details` pada 500 (string debug, hanya `NODE_ENV=development`), `details` pada 409 ini **selalu muncul** bila kolom yang melanggar dapat diekstrak; bila metadata tidak tersedia, field `details` tidak disertakan. Berlaku untuk `/create`, `/update`, `/adjust`, `/create-composite`, dan `/update-composite`.
...
**Field `details` (pada 500):** Berbentuk **string**, hanya muncul jika `NODE_ENV=development`. Berisi pesan error asli untuk debugging. Catatan: pada **409 Duplicate entry**, `details` berbentuk **object** (map kolom→pesan) dan selalu muncul bila kolom yang melanggar dapat diekstrak — lihat bagian 409.
...
**Scope:** Hanya endpoint `/change-status` dengan workflow hook bertipe API call (`workflow.hooks` di RDF). Penolakan dari handler component `onBeforeWorkflow`/`onAfterWorkflow` bukan 502, melainkan **400** `Workflow hook rejected`.
...
Elemen di dalam `conditions` boleh berupa grup, yaitu object `{ logic, conditions }` tanpa `key`. Grup diproses menjadi sub-ekspresi berkurung, sehingga kombinasi `AND` dan `OR` dapat ditulis tanpa memecah request:
...
Dihasilkan SQL:
```sql
WHERE (is_active = $1 AND (uom = $2 OR uom = $3))
```
...
| Aturan | Keterangan |
|--------|------------|
| Bentuk grup | Object dengan `logic` bernilai `AND` atau `OR` dan `conditions` berupa array non-empty. Object tanpa `key` tetapi punya `conditions` selalu diperlakukan sebagai grup |
| Isi grup | Kondisi biasa maupun grup lain, dengan operator dan opsi `sensitive` yang sama seperti format kompleks |
| Whitelist kolom | Berlaku sampai ke dalam grup. Kolom di luar whitelist endpoint ditolak `400` sama seperti pada kondisi level atas |
| Batas kedalaman | Maksimum 10 level bersarang |

Grup yang tidak memenuhi bentuk di atas ditolak `400` dengan pesan `Invalid where format: nested group must have logic AND/OR and a non-empty conditions array`. Bersarang melebihi batas ditolak `400` dengan pesan `Invalid where format: nested group depth exceeds maximum of 10`.

> **Catatan:** Pada endpoint yang membangun WHERE lewat model (`/read`, `/datatables`, `/lookup`, `/read-composite`), kedua pesan itu sampai ke client dalam bentuk terbungkus `Invalid where conditions: <pesan asli>`, mengikuti pola pesan error WHERE yang sudah berlaku di endpoint tersebut.
...
| File Storage | [../features/file-storage/](../features/file-storage/README.md) | Storage provider S3, alur upload, dan struktur metadata file |
