...
> **Catatan:** Kolom audit yang ditetapkan adalah `updated_at` dan `updated_by` (bukan kolom legacy `modified_by`/`modified_date`).
...
> Field yang terdaftar di `adjustConfig.fields` harus berupa kolom fisik tabel (subset `schemaFields`). Alias dari view atau `viewQuery` tidak dapat di-adjust karena endpoint `/adjust` menulis langsung ke tabel fisik.
