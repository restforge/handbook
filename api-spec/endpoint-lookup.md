...
| **Static dengan select/sort** | Body berisi `select` atau `sort_columns` (tanpa `where`) | Load semua data dengan kolom atau urutan custom |
| **Static legacy** | Body berisi `selected_tag` saja (atau body minimal) | Load semua data, tandai item yang dipilih |
...
|-----------|------|:-----:|------------|
| `where` | array / object | **Ya** (untuk mode ini) | Kondisi filter. Lihat [Format WHERE](README.md#format-where-clause) |
| `select` | array | Tidak | Kolom yang ditampilkan. Mendukung SQL expression (`\|\|`, `AS`, `CONCAT`) |
...
### Response Error POST (Error Response)
...
> **Catatan:** Validasi select memiliki **pengecualian**. Field yang mengandung SQL expression (`||`, `CONCAT`, `COALESCE`, `CASE`, `WHEN`) atau alias (`AS`) **tidak divalidasi** terhadap `validFields`.
...
## Struktur Data Response (Response Data Structure)
...
### Konfigurasi Kolom Lookup (Lookup Column Configuration)
...
**Contoh:** Jika payload mengonfigurasi `defaultScope.lookup: { is_active: true }`, maka semua query lookup otomatis mendapat kondisi `WHERE is_active = true`.
