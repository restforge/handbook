...
Fitur import memungkinkan data dari file Excel (.xlsx) dimasukkan ke database secara batch. Flow utama terdiri dari 3 langkah (upload, preview, commit) yang berjalan asinkron. Endpoint `/import-status` digunakan untuk polling progress di antara langkah-langkah tersebut, baik setelah preview maupun setelah commit. Mendukung auto-generate UUID, lookup field resolution (nama → ID), dan konfigurasi upsert strategy.
