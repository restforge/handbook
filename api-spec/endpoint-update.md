...
Jika distributed lock diaktifkan, sistem akan mengakuisisi per-record WRITE lock sebelum memulai operasi update dan melepaskannya setelah selesai (baik sukses maupun gagal). Hal ini mencegah konflik ketika dua request mencoba mengubah record yang sama secara bersamaan.
...
Terjadi ketika record sedang diproses oleh request lain dan distributed lock aktif. Kegagalan akuisisi lock ditangani sebagai error 500 (catch-all).
...
Terjadi ketika primary key yang diberikan tidak cocok dengan record manapun, atau update menghasilkan 0 rows affected.
...
Audit columns bersifat auto-managed dengan nama default `updated_at` dan `updated_by`. Operasi update hanya menyentuh dua kolom ini:

| Field | Nilai Otomatis | Kondisi |
|-------|---------------|---------|
| `updated_at` | SQL `CURRENT_TIMESTAMP` (waktu server) | Selalu di-set saat update |
| `updated_by` | user_id hasil resolusi berurutan: nilai di request body → header `x-user-id` → fallback `"system"` | Selalu di-set saat update |

> **Catatan:** Field `created_at` dan `created_by` **tidak** diubah oleh operasi update. Nama kolom dan nilai mengikuti konvensi audit columns default. Kolom legacy `modified_by`/`modified_date` tidak lagi di-handle runtime.
...
1. Mengakuisisi WRITE lock untuk record spesifik (`{project}:{endpoint}:{recordId}`)
2. Menjalankan operasi UPDATE
3. Melepaskan lock di akhir operasi (baik sukses maupun error)

Jika lock gagal diakuisisi (record sedang diproses request lain), kegagalan tersebut ditangani sebagai error **500** (lihat bagian "500 — Resource Sedang Dikunci" di atas).
