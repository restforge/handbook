...
| `remarks` | string | Tidak | Catatan terkait perubahan status. Disimpan jika `remarks` merupakan kolom tabel yang dapat ditulis |
| `updated_by` | string | Tidak | User yang melakukan perubahan. Disimpan jika `updated_by` merupakan kolom tabel yang dapat ditulis |
...
| `data` | object | Record setelah status diubah, berisi field yang terdaftar di `fieldName` |
...
#### 400 — Penolakan Hook Component (Business Rule)

Terjadi ketika handler component `onBeforeWorkflow` atau `onAfterWorkflow` mengembalikan `{ success: false, message }`. Transaksi di-rollback dan status tidak berubah. `message` berisi pesan asli dari handler, tanpa prefiks nama fungsi atau path file handler; detail lengkapnya hanya ditulis ke log server.

```json
{
  "success": false,
  "error": "Workflow hook rejected",
  "message": "Stok tidak mencukupi: Baju 29 (tersedia 0, diminta 5)",
  "timestamp": "2026-04-03T10:00:00.000Z"
}
```

Kode 400 ini dipakai konsisten dengan penolakan hook pada `/create`, `/update`, `/adjust`, `/delete`, dan endpoint composite. Berbeda dari 502 di bawah, yang khusus untuk hook workflow bertipe API call.
...
Terjadi ketika hook workflow bertipe API call (`workflow.hooks` di RDF) dengan `blocking: true` mengembalikan error. Penolakan dari handler component bukan 502, melainkan 400 `Workflow hook rejected` di atas.
...
| Property | Tipe | Default | Keterangan |
|----------|------|---------|------------|
| `statusField` | string | `"status"` | Nama kolom database yang menyimpan status. Harus terdaftar di `fieldName` dan merupakan kolom fisik tabel, karena runtime menolak kolom yang tidak dapat ditulis |
