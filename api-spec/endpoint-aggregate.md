...
| `operations[].field` | string | Ya | — | Nama field dari `readableFields` atau `"*"` (`"*"` hanya untuk `count`). Field hasil join tidak boleh dipakai di sini |
...
| `group_by` | array | Tidak | `[]` | Kolom untuk pengelompokan. Boleh field dari `readableFields` atau field hasil join yang didefinisikan di `aggregateConfig` |
| `where` | object | Tidak | — | Kondisi filter. Key harus ada di `readableFields`; kolom tidak dikenal ditolak 400 (fail-closed). Lihat [Format WHERE](README.md#format-where-clause) |
| `having` | array | Tidak | `[]` | Filter hasil agregasi (memerlukan `group_by`) |
| `having[].function` | string | Ya | — | `count`, `sum`, `avg`, `min`, `max` |
| `having[].field` | string | Ya | — | Nama field dari `readableFields` atau `"*"`. Field hasil join tidak boleh dipakai di sini, sama seperti `operations` |
...
> **Catatan field:** Field hasil join hanya boleh digunakan pada `group_by`. Untuk `operations` dan `having`, field wajib berupa field dari `readableFields` atau `"*"`. Untuk `where`, hanya field dari `readableFields` yang diizinkan (kolom join tidak dapat digunakan di `where`).
...
| `fields` | array | Kolom dari tabel join yang tersedia untuk `group_by`. Tidak berlaku untuk `operations`, `having`, maupun `where` |
