...
| `select` | array | Tidak | Kolom spesifik yang dikembalikan. Default: seluruh field yang terdaftar di `fieldName`. Kolom di luar `fieldName` tidak pernah dikembalikan |
...
| Hasil | Maks 1 record (`LIMIT 1`) | Sampai `READ_MAX_LIMIT` (default 5000), mode tanpa batas opsional |
| Paginasi | Tidak | Ya |
| Cache | Tidak | Ya |
