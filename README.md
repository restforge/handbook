...
Setiap project RESTForge menempati satu folder tersendiri. Folder tersebut menjadi root project yang memuat `node_modules`, `config/`, `schema/`, dan `payload/`. Karena instalasi bersifat lokal pada folder (bukan global), project baru selalu berarti folder baru. Instalasi global (`npm install -g`) tidak didukung dan akan ditolak dengan pesan error yang mengarahkan kembali ke instalasi lokal.

Buat folder project, masuk ke dalamnya, lalu install:

```bash
npx create-restforge-app my-project --yes
cd my-project

```

Hasilnya akan terbentuk folder project dengan nama `my-project` yang didalamnya sudah terinstall  RESTForge. Seluruh perintah `npx restforge` berikutnya, termasuk `init`, dijalankan dari dalam folder project ini.
...
| [`mcp/`](./mcp/) | Spec MCP server `@restforgejs/mcp-server`: daftar tool per domain, verb CLI yang dibungkus, parameter, dan catatan perilaku |
| [`features/`](./features/) | Penjelasan fitur runtime dan generator: component engine, dashboard, default scope, distributed lock, file storage, id generator, live sync, processor, query declarative |
| [`quickstart/`](./quickstart/) | Panduan mulai cepat dari instalasi sampai endpoint pertama berjalan |
...
|-------|-----------|-----------|
| **Edit source langsung** | Modifikasi file hasil generate | File lama otomatis diarsipkan ke `.restforge/archive/<YYYYMMDD-HHMMSS>/` saat regenerate |
