...
| [`restforge-backend/internal-binary.md`](./restforge-backend/internal-binary.md) | Dokumentasi 2 binary Kafka consumer (`restforge-consumer`, `restforge-consumer-deploy`) |
...
Pattern: `restforge-designer <verb> [options]`. Total **9 verb** (1 di antaranya parent dengan subcommand: `plugins`).
...
| Halaman | Fungsi |
|---------|--------|
| [`restforge-frontend/conventions.md`](./restforge-frontend/conventions.md) | Konvensi flag dan env var |
...
## Hubungan dengan MCP Server

Mayoritas verb di halaman ini juga tersedia sebagai tool MCP lewat `@restforgejs/mcp-server`, yang membungkus CLI yang sama dan menjalankannya di working directory project. Daftar tool per domain, verb CLI yang dibungkus, parameter, dan catatan perilaku ada di [`mcp/`](../mcp/). Halaman `commands/` tetap menjadi rujukan untuk kontrak flag CLI; halaman `mcp/` menjelaskan bentuk parameter tool dan perbedaan perilaku yang muncul karena pembungkusan tersebut.

---

**Lihat juga**: [`catalogs/`](../catalogs/) · [`mcp/`](../mcp/) · [`README`](../README.md)
