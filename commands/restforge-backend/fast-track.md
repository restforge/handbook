...
Pada mode `--overwrite`, file lama (RDF, endpoint, frontend) diarsipkan lebih dulu sebelum ditimpa. Seluruh langkah fast-track menulis ke satu folder run yang sama, misalnya `.restforge/archive/20260924-101500/payload/visitors.json`, sehingga perubahan file dapat dipulihkan (lihat [Arsip File Lama](./conventions.md#arsip-file-lama)). Penghapusan tabel tetap destruktif karena data di database tidak ikut diarsipkan.
