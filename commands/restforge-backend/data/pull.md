...
> Export isi tabel database ke file envelope JSON berdasarkan metadata SDF. Tabel
> ber-schema ditulis bersarang (`data-storage/<schema>/<table>.json`), tabel tanpa
> schema ditulis rata (`data-storage/<table>.json`). Mendukung satu tabel (`--table`),
> per schema (`--schema`), atau seluruh tabel terdaftar (`--all-schemas`).
...
4. **Load SDF + table-guard** sebelum koneksi database. Metadata kolom, namespace, dan
   primary key seluruhnya dibaca dari definisi SDF. Pada `--table`, tabel tidak terdaftar / ambigu →
