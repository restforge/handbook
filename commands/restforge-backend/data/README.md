...
- Daftar kolom, tipe canonical, nullable, dan primary key seluruhnya dibaca dari definisi SDF.
  Kolom yang tidak dideklarasikan di SDF tidak ikut di-pull dan ditolak saat push.
- Karena SDF hanya menyimpan 11 tipe canonical (`string`, `text`, `integer`, `bigint`,
  `decimal`, `boolean`, `date`, `timestamp`, `timestamptz`, `uuid`, `json`), kolom binary
  mustahil muncul (eksklusi by-omission). Tidak ada introspeksi database untuk mendeteksi tipe.
...
| `timestamptz` | string ISO-8601 dengan offset, dipertahankan apa adanya (hanya PostgreSQL) | `"2026-06-04T08:30:00.483+07:00"` |
