...
| `--force` | `false` | Timpa file existing di output directory |
| `--create-examples` | `true` | Generate example files (curl, Postman, Insomnia) |
| `--skip-sql-validation` | `false` | Lewati validasi keyword SQL |
| `--no-audit-migration` | `false` | Skip eksekusi audit table migration saat payload pakai `fieldPolicy` strategy `"audit"` (file SQL tetap ditulis ke `migrations/audit/`). Berguna untuk CI/CD pipeline atau production deployment yang ingin review DDL dulu sebelum eksekusi |
...
Untuk kondisi extraordinary seperti database offline, maintenance, atau
...
Saat flag ini aktif, validasi schema database di-skip sepenuhnya. Flag
`--config` boleh kosong dalam mode ini karena validator early-return
sebelum membaca config.

## Validasi Schema Database

Sejak versi 4.3.0, command `endpoint create` melakukan cross-check antara
...
output apapun. File existing tidak di-archive (tidak ada rotate `*.archive.NNN`).
...
3. Set `"auditColumns": false` di payload secara manual bila ingin override
   tanpa menjalankan sync

Sejak versi 4.5.0, `payload sync` resolve `auditColumns` secara otomatis:
bila tabel tidak punya kolom audit standar, command akan men-set
