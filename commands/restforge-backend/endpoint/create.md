...
| `--payload <FILE>` | - | Nama file payload JSON, dengan atau tanpa `.json`. Nama diubah menjadi huruf kecil oleh CLI, sehingga `Users` dicari sebagai `users.json`. Bentuk path ditolak; file dicari di `payload/` lalu di root working directory |
...
| `--force` | `false` | Timpa file yang sudah ada di output directory. Versi lama diarsipkan ke `.restforge/archive/` ([Arsip File Lama](../conventions.md#arsip-file-lama)) |
...
`--config` boleh kosong dalam mode ini karena file config tidak
dibaca sama sekali.

Flag ini tidak melewati validasi shape RDF. Payload dibaca dan diperiksa
strukturnya lebih dulu, sehingga error struktur seperti entri
`datatablesWhere` yang tidak valid tetap menghentikan command walau
`--skip-schema-check` dipakai.
...
output apa pun. File yang sudah ada tidak diarsipkan dan tidak ada folder run baru di `.restforge/archive/`.

Sebelum cross-check ini dijalankan, payload dibaca dan divalidasi strukturnya.
Kegagalan di tahap tersebut menghentikan command sebelum satu file pun ditulis,
dan tidak dapat dilewati dengan `--skip-schema-check` karena flag itu hanya
menyasar validasi schema database.

> **Peringatan upgrade:** Payload lapangan yang menulis `datatablesWhere` dengan
> prefiks alias tabel (misalnya `a.supplier_code`) sebelumnya lolos generate walau
> entri itu tidak pernah berfungsi saat pencarian. Payload semacam itu sekarang
> menghentikan `endpoint create` di tahap validasi struktur. Langkah pembersihannya:
> jalankan [`payload validate`](../payload/validate.md#logika-validasi) untuk melihat
> file yang bermasalah, lalu ganti entri beralias dengan nama kolom hasil SELECT yang
> disebutkan pesan error. Aturan lengkapnya ada di
> [Aturan `datatablesWhere`](../../../catalogs/rdf/validation-rules.md#aturan-datatableswhere).

Detail aturan validasi lihat [`catalogs/rdf/validation-rules.md`](../../../catalogs/rdf/validation-rules.md).

## Auth Guard

Bila payload memuat blok [`authGuard`](../../../catalogs/rdf/auth-guard.md)
dengan `enabled: true`, command turut men-generate middleware proteksi
`src/plugins/<project>-auth-guard.js` dan module loader memuatnya secara
otomatis sebelum plugin custom. Tanpa blok tersebut, output generate tidak
berubah sama sekali.
