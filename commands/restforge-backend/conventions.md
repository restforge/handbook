...
Sebelumnya beberapa schema command menerima `<path>` sebagai positional argument (mis. `schema validate ./schema`). Setelah refactor flag-only, sintaks positional dihapus sepenuhnya (hard-break, tanpa backward compatibility). Pemanggilan dengan sintaks lama akan menghasilkan error parser `Missing required flag: --path`.

## Penamaan Lokasi SDF (SDF Path Naming)

Lokasi file/folder SDF dirujuk dengan dua flag berbeda **secara sengaja**, sesuai peran command:

- `--path` — command primitif resource `schema` (`init`, `validate`, `models`, `generate-ddl`, `migrate`, `diff`, `apply`, `template`) yang argumen path-nya hanya SDF, sehingga nama generik tidak menimbulkan ambiguitas.
- `--schema-path` — command tingkat-aplikasi / orchestrator (`data pull`, `data push`, `fast-track`) yang memproses SDF bersama artefak lain (mis. `--storage-path` untuk file data), sehingga lokasi SDF perlu nama eksplisit agar tidak rancu dengan path lain.
...
- **Flag `--force`** bersifat **destructive** dan harus digunakan dengan hati-hati karena file yang sudah ada akan tertimpa tanpa konfirmasi.
