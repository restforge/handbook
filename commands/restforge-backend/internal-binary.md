...
> Binary untuk menjalankan dan men-deploy Kafka consumer hasil generate `kafka consumer-create`.

Binary pada bagian ini tidak mengikuti pattern resource-first dan berdiri sendiri tanpa prefix `npx restforge`. Pemanggilan menggunakan binary independen (`restforge-consumer`, `restforge-consumer-deploy`).
