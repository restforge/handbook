...
Sejak versi 4.5.0, `payload diff` memakai source-of-truth tunggal dengan
`endpoint create` dan `payload validate`/`sync`: comparator audit-column-aware
`compareSchemaStrict`. Berbeda dengan `validate` yang hanya tampilkan verdict,
...
| `[+] created_at, ...` | Kolom audit di-required oleh `auditColumns=true` tapi tidak di database |
