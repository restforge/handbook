...
| `--overwrite` | `false` | Timpa file output jika sudah ada. Gate berlaku untuk setiap file split yang sudah ada di output directory. File yang isinya berubah diarsipkan lebih dulu ke `.restforge/archive/` |
...
| Fitur otomatis (status filter) | Field status utama — `is_active` boolean ATAU `is_active`/`status` dengan enum RDF — → `enableStatusFilter: true` + `statusFilter` (BUKAN lagi "boolean saja", lihat [`catalogs/udf/features.md`](../../../catalogs/udf/features.md#enablestatusfilter-dan-statusfilter)) |
...
Field bernama `is_active` atau `status` yang beresolusi boolean (checkbox) ATAU
select+enum (`constraints.enum` di RDF) otomatis menjadi field status utama:
`enableStatusFilter: true` dengan `statusFilter.options` sesuai tipenya. Untuk
...
#### Preservasi Atribut Lookup Saat `--overwrite`

Migrate ulang dengan `--overwrite` menulis ulang `dataSource` field FK dari
skema database, sehingga atribut lookup yang ditambahkan manual pasca-migrate
sebelumnya (lihat [Atribut Lookup Tambahan](../../../catalogs/udf/data-source.md#atribut-lookup-tambahan-autofill-optioncolumns-lookupdisplay))
berisiko hilang setiap kali migrate dijalankan ulang. Untuk mencegah ini,
migrate mempertahankan tiga blok berikut pada `dataSource` field select
ber-lookup, baik pada `pages[].fields[]` (halaman master, dipasangkan lewat
nama halaman dan nama field) maupun pada `details[].fields[]` (grid detail,
dipasangkan lewat nama halaman, nama detail, dan nama field), dengan
membandingkan UDF lama di disk terhadap hasil migrate baru:

| Blok | Dipertahankan |
|------|:-------------:|
| `dataSource.autofill` | ✓ |
| `dataSource.optionColumns` | ✓ |
| `dataSource.lookupDisplay` | ✓ |
| `defaultValue` pada field (di luar `dataSource`) | ✗ |

Entri `features.dataFilters[]` tidak tersentuh mekanisme ini sama sekali —
preservasi hanya berlaku pada field `select` di `pages[].fields[]` dan
`details[].fields[]`.

Kolom yang dirujuk `autofill`/`optionColumns` (mis. `price`) turut ditambahkan
ke `dataSource.select` bila belum ada, sehingga endpoint lookup tetap diminta
membawa kolom yang dibutuhkan tanpa perlu penyuntingan manual ulang — kolom
`select` yang sudah ada dan urutannya tidak disentuh, hanya ditambahkan di
belakang.

Field yang sudah tidak ada lagi di schema (hilang dari hasil migrate baru)
tidak ikut mempertahankan bloknya. Console menampilkan baris `Preserved
dataSource block(s): <daftar field>` untuk setiap field yang blok lookup-nya
dipertahankan — format `<pageId>.<fieldName>` untuk field halaman master,
`<pageId>.<detailId>.<fieldName>` untuk field grid detail.

`defaultValue` pada field (lihat
[Nilai Awal Baris Baru](../../../catalogs/udf/master-detail.md#nilai-awal-baris-baru-defaultvalue)
untuk konteks grid detail) bukan bagian dari preservasi ini di kedua konteks,
karena posisinya di luar `dataSource` dan tidak berasal dari metadata RDF
apa pun — sama seperti `autoCalculateFields` RDF, atribut ini perlu ditulis
ulang secara manual di UDF setelah migrate dengan `--overwrite`.
