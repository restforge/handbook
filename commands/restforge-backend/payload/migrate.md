...
diuraikan pada section [Output Split Multi-File](#output-split-multi-file) dan
...
sibling folder payload dan mengkonversinya menjadi page tersendiri. Satu RDF utama
ber-JOIN dapat menghasilkan beberapa page sekaligus.

Mekanisme:

1. Migrator mem-parse JOIN pada `datatablesQuery` RDF utama (referensi `file:` di-resolve
   lebih dulu).
...
| JOIN discovery | RDF utama ber-JOIN memuat RDF tabel terkait dari sibling folder dan mengkonversinya menjadi page tambahan (1 RDF utama → multi page) |
...
| SQL parser | Parser memakai regex untuk pattern standar. Query dengan subquery, CASE, atau aggregate function mungkin tidak ter-parse sempurna |
...
Sebelumnya migrasi RDF → UDF dilakukan via `restforge-designer migrate` (binary frontend). Command tersebut sudah **di-drop** dan digantikan oleh `npx restforge payload migrate` (binary backend). Alasan: konversi RDF → UDF memerlukan akses ke config database backend (mis. `SERVER_PORT`, `SERVER_ADDRESS`) yang lebih natural di-handle di sisi backend.
