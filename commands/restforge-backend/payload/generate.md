...
Constraint tambahan seperti `format`, `pattern`, `min`/`max`, `enum`, atau preset `email`/`phone`/`url`/`uuid` perlu ditambahkan secara manual di file payload setelah generate. Detail constraint per tipe ada di [`catalogs/rdf/field-validation`](../../../catalogs/rdf/field-validation.md).
