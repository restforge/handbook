# `npx restforge-designer validate`
...
```bash
npx restforge-designer validate --payload <PAYLOAD> [OPTIONS]
```
...
```bash
npx restforge-designer validate --payload=payload/contact-mgmt.json
```
...
```bash
npx restforge-designer validate --payload=payload/contact-mgmt.json --plugins-dir=./my-plugins
```
...
| CI/CD pre-merge check | `validate` (cepat) |
