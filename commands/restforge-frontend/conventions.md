...
| `RESTFORGE_ARCHIVE_RETAIN` | Jumlah folder run arsip yang disimpan, sama dengan flag `--archive-retain=<n>` |

## Arsip File Lama

Verb yang menimpa file dengan `--overwrite` (`generate` dan `auth`) memindahkan versi lama ke folder `.restforge/archive/<YYYYMMDD-HHMMSS>/` di root project, dengan path relatif dan nama file asli dipertahankan. Flag global `--archive-retain=<n>` mengatur jumlah folder run yang disimpan (default `5`, `0` berarti simpan semua).

Root project adalah folder tempat command dijalankan. Bila output berada di luar folder itu, root project diambil dari folder induk terdekat yang memiliki folder `.restforge/`. Bila tidak ada, arsip ditulis ke `.restforge/archive/` di dalam folder output.

Konvensi lengkap, termasuk isi `manifest.json` dan cara memulihkan satu run, ada di [Arsip File Lama pada konvensi backend](../restforge-backend/conventions.md#arsip-file-lama).
