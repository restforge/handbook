...
| `fields` | Whitelist tipe field, editor mode, opsi id-generation, badge color, range `decimalPlaces`, plus pola `temporalType`/`dateFormat` field tanggal |
| `navigation` | Aturan navigasi: tipe item, kedalaman maksimum, pattern `pageId` |
| `appConfig` | Required fields `appConfig` + range `port` + `numberFormat.locale`/`currencyPrefix` + pola `dateFormat`/`dateTimeFormat` |
...
| `app-config` | `appConfig` | Saat mengisi `appConfig` (field wajib, range port, locale/currencyPrefix format angka) |
...
```json
{
  "appConfig": {
    ...
    "numberFormat": {
      "locale": { "allowed": ["en-US", "id-ID"] },
      "currencyPrefix": { "type": "string" }
    },
    "dateFormat": {
      "allowed": ["yyyy-MM-dd", "dd/MM/yyyy", "dd-MM-yyyy", "MM/dd/yyyy", "yyyy/MM/dd"],
      "default": "yyyy-MM-dd",
      "note": "Must match DATEFORMAT of the backend"
    },
    "dateTimeFormat": {
      "datePatterns": ["yyyy-MM-dd", "dd/MM/yyyy", "dd-MM-yyyy", "MM/dd/yyyy", "yyyy/MM/dd"],
      "timePatterns": ["HH:mm", "HH:mm:ss", "HH:mm:ss.SSS"],
      "default": "yyyy-MM-dd HH:mm:ss.SSS",
      "note": "'<date pattern> <time pattern>'; must match DATETIMEFORMAT of the backend"
    }
  }
}
```
