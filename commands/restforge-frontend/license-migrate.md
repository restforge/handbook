...
|-------------------|--------------------------|
| Plain text — license terbaca siapa pun yang akses file | Disimpan terenkripsi di OS keyring atau encrypted file fallback |
...
Jika key di `.env` sama dengan key di encrypted storage, command memperlakukannya sebagai idempotent:
