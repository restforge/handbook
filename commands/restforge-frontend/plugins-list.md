# `npx restforge-designer plugins list`
...
```bash
npx restforge-designer plugins list [OPTIONS]
```
...
Use `npx restforge-designer plugins inspect --plugin=<id>` for details.
```

### List dari Plugins Dir Custom

```bash
npx restforge-designer plugins list --plugins-dir=./my-plugins
```
...
3. **Muat metadata** masing-masing plugin
...
Pakai `npx restforge-designer plugins list --plugins-dir=<custom-path>` untuk override.
